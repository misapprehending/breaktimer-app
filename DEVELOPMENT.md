...
That publishes `LAQBreakTimer-Setup-<version>.exe` as a GitHub Release for IT to download and deploy with Intune. The app does not auto-update from GitHub. Tagging `v*` also runs `.github/workflows/release.yml`. The workflow uploads only the `.exe` (not `.blockmap` or `latest.yml`).
